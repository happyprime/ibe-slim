...
function HeaderToolbar(props) {
	...
	const {
		...
		return {
			// @ts-ignore
			fixedToolbar:
				select('isolated/editor').isFeatureActive('fixedToolbar'),
			...
			// @ts-ignore
			isTextModeEnabled:
				select('isolated/editor').getEditorMode() === 'text',
	...
	const { inserter, navigation, undo, selectorTool } =
		props.settings.iso.toolbar;
	const inserterInSidebar = props.settings?.iso?.sidebar?.inserter || false;
	const displayBlockToolbar =
		!isLargeViewport || previewDeviceType !== 'Desktop' || fixedToolbar;

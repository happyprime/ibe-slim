...
async function getInitialContent(settings, loader) {
	const contentLoader =
		loader && typeof loader.then === 'function'
			? loader
			: new Promise((resolve) => {
...
function BlockEditorContents(props) {
	const { blocks, onInput, onChange, selection, isEditing, editorMode } =
		props;
...
export default compose([
	withSelect((select, ownProps) => {
		const { getBlocks, getEditorSelection, getEditorMode, isEditing } =
	...
	withDispatch((dispatch, ownProps) => {
		const { updateBlocksWithUndo, updateBlocksWithoutUndo } =
